...
def make_all_summary_plots(events, output_prefix=""):
    data = extract_plot_data(events)

    global_residuals = data["residuals"]
    global_momentum_for_histogram = data["momenta"]
    global_distance_x = data["dx"]
    global_distance_y = data["dy"]
    global_px = data["px"]
    global_py = data["py"]
    global_pz = data["pz"]

    state_x = data["x_state"]
    state_y = data["y_state"]
